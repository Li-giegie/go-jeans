...
func EncodeFaster(buf []byte, args ...interface{}) ([]byte, error) {
	...
	for i := 0; i < len(args); i++ {
		...
		case []uint:
			...
			if length != 0 {
				for j = 0; j < length; j++ {
					buf = binary.LittleEndian.AppendUint64(buf, uint64(v[j]))
				}
			}
		case []int:
			length = len(v)
		...
		case []uint16:
			...
			if length != 0 {
				for j = 0; j < length; j++ {
					buf = binary.LittleEndian.AppendUint16(buf, v[j])
				}
			}
		case []uint32:
			length = len(v)
			...
			if length != 0 {
				for j = 0; j < length; j++ {
					buf = binary.LittleEndian.AppendUint32(buf, v[j])
				}
			}
		case []uint64:
			length = len(v)
			...
			if length != 0 {
				for j = 0; j < length; j++ {
					buf = binary.LittleEndian.AppendUint64(buf, v[j])
				}
			}
		case []int8:
			length = len(v)
		...
		case []int16:
			...
			if length != 0 {
				for j = 0; j < length; j++ {
					buf = binary.LittleEndian.AppendUint16(buf, uint16(v[j]))
				}
			}
		case []int32:
			length = len(v)
			...
			if length != 0 {
				for j = 0; j < length; j++ {
					buf = binary.LittleEndian.AppendUint32(buf, uint32(v[j]))
				}
			}
		case []int64:
			length = len(v)
		...
		case []float32:
			...
			if length != 0 {
				for j = 0; j < length; j++ {
					buf = binary.LittleEndian.AppendUint32(buf, math.Float32bits(v[j]))
				}
			}
		case []float64:
			length = len(v)
			...
			if length != 0 {
				for j = 0; j < length; j++ {
					buf = binary.LittleEndian.AppendUint64(buf, math.Float64bits(v[j]))
				}
			}
		case []bool:
			length = len(v)
